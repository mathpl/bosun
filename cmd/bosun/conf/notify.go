...
func (n *Notification) Notify(subject, body, httpBody string, emailsubject, emailbody []byte, c *Conf, ak string, attachments ...*models.Attachment) {
	if len(n.Email) > 0 {
		go n.DoEmail(emailsubject, emailbody, c, ak, attachments...)
	}
	if n.Post != nil {
		if httpBody != "" {
			go n.DoPost([]byte(httpBody), ak)
		} else {
